...
bad_sum_image=sitk.ReadImage('./output/bad_label_sum.nii')
bad_sum_data=sitk.GetArrayFromImage(bad_sum_image)

medium_sum_image=sitk.ReadImage('./output/medium_label_sum.nii')
medium_sum_data=sitk.GetArrayFromImage(medium_sum_image)

slight_sum_image=sitk.ReadImage('./output/slight_label_sum.nii')
slight_sum_data=sitk.GetArrayFromImage(slight_sum_image)

none_sum_image=sitk.ReadImage('./output/none_label_sum.nii')
none_sum_data=sitk.GetArrayFromImage(none_sum_image)

mr_35_image=sitk.ReadImage('./moving/aligned_seg35.nii.gz')
