...
for train_index,test_index in skf.split(X,y):
    ...
    probas_trian=pipe.predict_proba(X_train)
    #print(probas_[:,1])
    #print(pipe['clf'].coef_)
    importances.append(abs(pipe['clf'].coef_))
    #概率调整
...
# plt.plot(mean_fpr, mean_tpr, color='navy',label='Mean ROC (auc = {0:.2f})'.format(np.mean(auc)), lw=2)
...
#     final_weight.setdefault(name,0)

# #print(final_weight.keys())

# for  key,j in zip(final_weight.keys(),range(0,43)):
#     for i in range(0,10):
#         final_weight[key]+=importances[i][0][j]
   
# #print(sorted(final_weight.items(),key=lambda x:x[1],reverse=True))

# sorted_weight_list=sorted(final_weight.items(),key=lambda x:x[1],reverse=True)[0:10]
# #print(importances)    
# weight_names_list=[]
# weight_data_list=[]
...
# for i in range(0,8):
#     j=7-i
#     weight_names_list.append(sorted_weight_list[j][0])
#     weight_data_list.append(sorted_weight_list[j][1])
# plt.barh(y=weight_names_list,width=weight_data_list,height=0.4,label='bleeding extrusion',color='red')
