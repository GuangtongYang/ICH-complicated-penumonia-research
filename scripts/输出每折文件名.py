...
name_file=open(r'',\
    'r')
patient_names=name_file.readlines();
name_file.close()
# data.describe()
...
pipe = Pipeline(steps=[ ('std',StandardScaler()),
                       ...
                       penalty='l1',C=0.1,multi_class='auto'))])
...
for train_index,test_index in skf.split(X,y):
    ...
    fw_train=open('./logistic_cv_distribution_2/train_'+str(count)+'.txt','a')
    fw_test=open('./logistic_cv_distribution_2/test_'+str(count)+'.txt','a')
    count+=1
    for index in train_index:
        name=re.search(r' .+{',patient_names[index]).group()[1:-1]
        fw_train.write(name+'\n')
    ...
    acc.append(accuracy_score(y[test_index],y_pred=y_pred_after_adjust))
    auc.append(roc_auc_score(y[test_index],y_score=probas_[:,1]))
    se.append(recall_score(y[test_index],y_pred=y_pred_after_adjust))
    PPV.append(precision_score(y[test_index],y_pred=y_pred_after_adjust))
    tn, fp, fn, tp = confusion_matrix(y[test_index], y_pred_after_adjust).ravel()
    sp.append(tn/(tn+fp))
    NPV.append(tn/(tn+fn))
